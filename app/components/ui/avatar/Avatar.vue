...
const px = computed(() => (typeof props.size === 'number' ? props.size : (SIZES[props.size] ?? 36)))

const cls = computed(() =>
  ['jl-avatar', props.square ? 'jl-avatar--square' : '', props.ring ? 'jl-avatar__ring' : '']
    .filter(Boolean)
    .join(' '),
)
...
<template>
  <span :class="cls" :style="style">
    ...
    <span v-if="props.status" :class="`jl-avatar__status jl-avatar__status--${props.status}`" />
  </span>
</template>
